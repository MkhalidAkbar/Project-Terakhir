...
WHERE tb_mengajar.id_guru AND tb_mengajar.id_mengajar='$_GET[pelajaran]'  AND tb_thajaran.status=1  ");
...
<a href="#">Kelas <?=strtoupper($d['nama_kelas']) ?></a>
									...
									<div class="row">
										...
										foreach ($qry as $pertemuan) { ?>
										<?php 
										$pertemuan = date('m',strtotime($pertemuan['tgl_absen']));
										?>

										<a target="_blank" href="modul/rekap/rekap_bulan.php?pelajaran=<?=$_GET['pelajaran'] ?>&bulan=<?=$pertemuan;?>&kelas=<?=$d['id_mkelas'] ?>" style="text-decoration: none;" class="text-primary">
										<div class="alert alert-primary alert-dismissible" role="alert">
										<strong>REKAP BULAN (<?= strtoupper(namaBulan($pertemuan)) ?> <?= strtoupper(date('Y')) ?>) </strong> 
										</div>
										</a>
					...
					<br>
								...
								<?php 
									...
									WHERE tb_walikelas.id_guru='$data[id_guru]'  and tb_walikelas.id_walikelas
									and tb_walikelas.id_mkelas
									ORDER BY tb_walikelas.id_mkelas");
				...
				<div class="row">
					<div class="col-md-12 col-xs-12">	
						<a href="?page=rekap&kelas=<?=$g['id_mkelas'] ?>&guru=<?=$g['id_guru'] ?>&pelajaran=<?=$g['id_mengajar'] ?>&bulan=<?=$bulan ?>&walikelas=<?=$g['id_walikelas'] ?>" style="text-decoration: none;" class="text-success">
							<div class="alert alert-success alert-dismissible" role="alert">
